...
from app.models import PRD, PROD #POD, PRD, PROD
import json
from app.utils import decoder, special_configuration #decoder, sql_function, special_configuration
import os
...
def daily_run(df, rev1, rev2, prodtype, warranty = False):
    ...
    ML_path = "/home/ubuntu/KW-AIRO-PartRecommendation-API/app/models/model_store/" + rev1 + ".pkl"
    label_encoder_path = "/home/ubuntu/KW-AIRO-PartRecommendation-API/app/models/model_store/" + rev1 + ".json"
    ...
    for v in pod_versions:
        ...
        if v == 'None':
            ...
            POD_path = f"/home/ubuntu/KW-AIRO-PartRecommendation-API/app/models/POD_jsons/{rev2}.json"
        else:
            POD_path = f"/home/ubuntu/KW-AIRO-PartRecommendation-API/app/models/POD_jsons/{rev2}_{v}.json"
        ...
        instruction_filepath = f'/home/ubuntu/KW-AIRO-PartRecommendation-API/app/data/instructions/{prodtype}.json'
        try:
        ...
        try:
            ...
            try:
                ...
                POD_path =  f"/home/ubuntu/KW-AIRO-PartRecommendation-API/app/models/POD_jsons/{rev2}.json"
                POD_json = js_r(POD_path)
                new_path = f"/home/ubuntu/KW-AIRO-PartRecommendation-API/app/models/POD_jsons/{rev2}_{v}.json"
...
# Multiple tickets
def batch_prodrun():
    directory = '/home/ubuntu/KW-AIRO-PartRecommendation-API/app/models/model_store/'
    
    ### Temporary Seal Accommodation
    seal_dict = js_r('/home/ubuntu/KW-AIRO-PartRecommendation-API/app/data/seal.json')
    ...
    test_row = pd.DataFrame({
        'ID': ['57556687', '57556688'],
        'WORKORDERID': ['57557809', '57557810'],
        'MODELNO': ['RF263BEAESG/AA', 'RF263BEAESG/AA'], 
        'VERSION': ['4', '4'],
        'MANUFACTUREMONTH': ['201708', '201708'],
        'SYMPTOMDESCRIPTION1': ['Ice / Water / Sparkling', 'Operation'],
        'SYMPTOMDESCRIPTION2': ['Ice making / Ice bucket stuck issue', 'Condensation'],
        'SYMPTOMDESCRIPTION3': ['Ice room or bucket frost', 'Condensation'],
        'STATUS': ['220', '220'],
        'PRODUCTTYPE': ['REF_REF', 'REF_REF']
        })

    non_pkl_tkts, df = pkl_file_check(df=test_row, file_name_list= file_names)
    ...
    print(f'Result: {final_model_result_df}')
...
def single_prodrun():
    ...
    test_row = pd.DataFrame({
        ...
        'SYMPTOMDESCRIPTION3': ['Ice room or bucket frost'],
        'STATUS': ['220'],
        'PRODUCTTYPE': ['REF_REF']
        })
